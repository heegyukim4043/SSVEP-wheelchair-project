function [flickerTexture]=flickerTexture_9class_fixed(winWidth,winHeight,targetWidth,targetHeight)
...
%     00100 Down
    for i =1:winHeight
        for j=1:winWidth
         %Target0 : index1
         if (j>= winWidth/4)&&( j <= winWidth*3/4)&&( i <= input_Height2/5)  &&(i >=1)
            targetMatrix{10}(i,j)=1; %match target coordinate
            %disp('got it');     
         end 
         if (j>= winWidth/4)&&( j <= winWidth*3/4)&&( i <= input_Height*4/5)  &&(i >=input_Height*3/5)
            targetMatrix{10}(i,j)=1; %match target coordinate
            %disp('got it');     
         end
            
        %Target1 : L-T
        if (j>= (1))&&( j <= targetWidth)&&( i <= targetHeight+input_Height)  &&(i >=input_Height)
            targetMatrix{1}(i,j)=1; %match target coordinate
            %disp('got it');     
        end
        %Target2 : M-T
        if (j>= (winWidth/2-targetWidth/2))&&( j <= winWidth/2+targetWidth/2)&& (i <= targetHeight+input_Height)  &&(i >=input_Height)
            targetMatrix{2}(i,j)=1; %match target coordinate
            %disp('got it');     
        end
        %Target3 : R-T
        if (j>= winWidth-targetWidth)&&( j <= winWidth)&& (i <= targetHeight+input_Height)  &&(i >=input_Height)
            targetMatrix{3}(i,j)=1; %match target coordinate
            %disp('got it');     
        end
        
        %Target4 : L-M
        if (j>= (1))&&( j <= targetWidth)&&(i>= (winHeight/2-targetHeight/2+input_Height/2))&&( i <= winHeight/2+targetHeight/2+input_Height/2)
            targetMatrix{4}(i,j)=1; %match target coordinate
            %disp('got it');     
        end
        %Target5 : M-M
        if (j>= (winWidth/2-targetWidth/2))&&( j <= winWidth/2+targetWidth/2)&&(i>= (winHeight/2-targetHeight/2+input_Height/2))&&( i <= winHeight/2+targetHeight/2+input_Height/2)
            targetMatrix{5}(i,j)=1; %match target coordinate
            %disp('got it');     
        end
        %Target6 : R-M
        if (j>= winWidth-targetWidth)&&( j <= winWidth)&&(i>= (winHeight/2-targetHeight/2+input_Height/2))&&( i <= winHeight/2+targetHeight/2+input_Height/2)
