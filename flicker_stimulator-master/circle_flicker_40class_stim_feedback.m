function circle_flicker_40class_stim_feedback(preload)

% circle_flicker_40class_stim_feedback(flicker_images)
%%Initiate frequency
...
freq_list = linspace(freq_min,freq_max,class);
...
% end
while (1)
    Openvibe_tcp(t, 5); %% start stm based epoch: 5
    WaitSecs(stimul_len +0.5);
    tcp_dat =read(s,srate*stimul_len,'double');
    % tcp_dat =readline(s,srate*stimul_len,'double');
%     tcp_dat =readline(s);
...
% eegplot(double(unique_nb),'srate',60);
...
try
    ...
    if (isempty(preload))
        screenMatrix = flickerTexture_all_class_circle(width, height, targetWidth, targetHeight,class,margin_x,margin_y, unique_nb',r_max,r_min);
        ...
        texture_name = sprintf('pre_texture_circle_%d_%d_freq_%.01f_%.01f.mat',r_min,r_max,freq_min,freq_max);
...
%             Screen('DrawText', win,'>>', true_pos(1),true_pos(2), 0);
    ...
    for trial_2 = 1: ceil(class/stimul_size)
        ...
        for trial_1  =1: length(stimul_set)
            ...
            Screen('DrawTexture',win,texture(end), [0 0 1920 1080], [0 0 1920 1080]);
            Screen('Flip', win);
            ...
            angle_diff = (pi/class)*(results(trial_1)-1) + pi;
            angles = linspace(0 + angle_diff, pi/class + angle_diff, max([width, height]) );
            x_feedback = round( (r_max + 50 ) * cos(mean(angles)) + width/2);
            y_feedback =round( (r_max + 50 ) * sin(mean(angles)) + height/2);
            

            Screen('DrawTexture',win,texture(end), [0 0 1920 1080], [0 0 1920 1080]);
            Screen('DrawText', win, '*', x_feedback , y_feedback ,[255 0 0]);
